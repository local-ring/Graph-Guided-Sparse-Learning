...
def minConF_PQN(funObj, x, funProj, options=None):
    ...
    default_options = {
        ...
        'suffDec': 1e-10,        # Sufficient decrease parameter for Armijo
    ...
    while funEvals <= maxIter:
        ...
        if i == 1:
            p = funProj(x-g)
            # print("p:", p, "p_max:", np.max(p), "p_min:", np.min(p), "number of p larger than eps:", np.sum(p > 1e-5))
            projects = projects + 1
            S = np.zeros((nVars, 0))
            Y = np.zeros((nVars, 0))
            Hdiag = 1
        ...
        d = p - x
        # print("d:", d, "d_max:", np.max(d), "d_min:", np.min(d), "number of d larger than eps:", np.sum(d > 1e-5))
        g_old = g
        # print("g_old:", g_old, "g_old_max:", np.max(g_old), "g_old_min:", np.min(g_old), "number of g_old larger than eps:", np.sum(np.abs(g_old) > 1e-5))
        x_old = x
        ...
        # while f_new.item() > f:
            ...
            x_new = x + t * d
            f_new, g_new = funObj(x_new)
            # print(f"Iteration {i}: Step size t = {t}, f_new = {f_new}, f_old = {f}, differnece: {f_new - f}")   
            funEvals = funEvals + 1
